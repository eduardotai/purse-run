...
export function resolveFight(
  ...
  function locate(instance: number): Located | null {
    ...
    if (playerIndex >= 0) {
      const fighter = player[playerIndex]
      if (fighter) return { side: "player", index: playerIndex, fighter }
    }
    const enemyIndex = enemy.findIndex((fighter) => fighter.instance === instance)
    if (enemyIndex >= 0) {
      const fighter = enemy[enemyIndex]
      if (fighter) return { side: "enemy", index: enemyIndex, fighter }
    }
    return null
  }
  ...
  while (!capped && living(player) && living(enemy)) {
    if (player[0] && enemy[0] && (player[0].health <= 0 || enemy[0].health <= 0)) {
      sweep(player)
      sweep(enemy)
      continue
    }
